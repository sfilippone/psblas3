...
9999 call psb_error_handler(ctxt,err_act)
  ...
  subroutine run_spmv_kernel(ctxt,use_gpu,matrix_file,matrix_fmt,cpu_fmt,gpu_fmt,idim_in,times_in,do_swap,comm_mode)
...
#ifdef PSB_HAVE_CUDA
    type(psb_s_vect_cuda)                     :: cuda_vector_mold
    type(psb_i_vect_cuda)                     :: cuda_index_mold
    type(psb_s_cuda_elg_sparse_mat), target   :: cuda_ell_sparse_mold
    type(psb_s_cuda_csrg_sparse_mat), target  :: cuda_csr_sparse_mold
    type(psb_s_cuda_hdiag_sparse_mat), target :: cuda_hdia_sparse_mold
    type(psb_s_cuda_hlg_sparse_mat), target   :: cuda_hll_sparse_mold
    class(psb_s_base_sparse_mat), pointer     :: cuda_sparse_mold
#endif
    ...
    if (use_external_matrix) then
      ...
      call psb_s_gen_pde3d(ctxt,idim,a,y,x,desc_a,afmt,info)
    end if
...
9999 call psb_error(ctxt)
  ...
  subroutine load_external_matrix(ctxt, matrix_file, matrix_fmt, a, bv, xv, desc_a, afmt, info)
    ...
    ! Reading it on every rank costs one full copy per rank -- at 448 ranks on
    ! 4 nodes that is over 100 GB per node for a 60M nonzero matrix, and the
    ! job is OOM-killed before it ever gets to the SpMV.
    if (iam == psb_root_) then
...
end module psb_sspmv_overlap_test

program psb_s_spmv_kernel
  use psb_sspmv_overlap_test
  use psb_base_mod
#ifdef PSB_HAVE_CUDA
  use psb_cuda_mod
#endif
  ...
  if (my_rank == psb_root_) then
    ...
    write(psb_out_unit,*) 'This is the psb_s_spmv_kernel sample program'
    write(psb_out_unit,'("GPU enabled          : ",l1)') use_gpu
    write(psb_out_unit,'("Usage: ./psb_s_spmv_kernel [--gpu=TRUE|FALSE] [--dim=N] [--times=N] ",&
