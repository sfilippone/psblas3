...
end module psb_dspmv_overlap_test

program psb_d_spmv_kernel
  use psb_dspmv_overlap_test
  use psb_base_mod
#ifdef PSB_HAVE_CUDA
  use psb_cuda_mod
#endif
  ...
  if (my_rank == psb_root_) then
    ...
    write(psb_out_unit,*) 'This is the psb_d_spmv_kernel sample program'
    write(psb_out_unit,'("GPU enabled          : ",l1)') use_gpu
    write(psb_out_unit,'("Usage: ./psb_d_spmv_kernel [--gpu=TRUE|FALSE] [--dim=N] [--times=N] ",&
