...
module psb_zspmv_overlap_test
  ...
  interface 
    function z_func_3d(x,y,z) result(val)
      import :: psb_dpk_
      real(psb_dpk_), intent(in) :: x,y,z
      complex(psb_dpk_) :: val
    end function z_func_3d
  end interface 

contains


  function z_null_func_3d(x,y,z) result(val)
  ...
  !  the rhs. 
  !
  subroutine psb_z_gen_pde3d(ctxt,idim,a,bv,xv,desc_a,afmt,info,&
    ...
    character(len=*)      :: afmt
    procedure(z_func_3d), optional :: f
    class(psb_z_base_sparse_mat), optional :: amold
    class(psb_z_base_vect_type), optional :: vmold 
    ...
    procedure(z_func_3d), pointer :: f_
    logical :: tnd_
...
9999 call psb_error_handler(ctxt,err_act)
    ...
    return
  end subroutine psb_z_gen_pde3d
  ...
  subroutine run_spmv_kernel(ctxt,use_gpu,matrix_file,matrix_fmt,cpu_fmt,gpu_fmt,idim_in,times_in,do_swap,comm_mode)
    ...
    real(psb_dpk_)                 :: t0, t1, dt, avg_t
    logical                         :: use_external_matrix

    integer(psb_ipk_)               :: comm_type

#ifdef PSB_HAVE_CUDA
    type(psb_z_vect_cuda)                     :: cuda_vector_mold
    type(psb_i_vect_cuda)                     :: cuda_index_mold
    type(psb_z_cuda_elg_sparse_mat), target   :: cuda_ell_sparse_mold
    type(psb_z_cuda_csrg_sparse_mat), target  :: cuda_csr_sparse_mold
    type(psb_z_cuda_hdiag_sparse_mat), target :: cuda_hdia_sparse_mold
    type(psb_z_cuda_hlg_sparse_mat), target   :: cuda_hll_sparse_mold
    class(psb_z_base_sparse_mat), pointer     :: cuda_sparse_mold
#endif
    ...
    if (use_external_matrix) then
      ...
      call psb_z_gen_pde3d(ctxt,idim,a,y,x,desc_a,afmt,info)
    end if
...
9999 call psb_error(ctxt)
  ...
  subroutine load_external_matrix(ctxt, matrix_file, matrix_fmt, a, bv, xv, desc_a, afmt, info)
    ...
    type(psb_z_vect_type), intent(out)   :: bv, xv
    type(psb_desc_type), intent(out)     :: desc_a
    character(len=*), intent(in)         :: afmt
    integer(psb_ipk_), intent(out)       :: info

    type(psb_lzspmat_type) :: aux_a
    complex(psb_dpk_), allocatable :: rhs_glob(:), x_glob(:)
...
end module psb_zspmv_overlap_test

program psb_z_spmv_kernel
  use psb_zspmv_overlap_test
  use psb_base_mod
#ifdef PSB_HAVE_CUDA
  use psb_cuda_mod
#endif
  ...
  if (my_rank == psb_root_) then
    ...
    write(psb_out_unit,*) 'This is the psb_z_spmv_kernel sample program'
    write(psb_out_unit,'("GPU enabled          : ",l1)') use_gpu
    write(psb_out_unit,'("Usage: ./psb_z_spmv_kernel [--gpu=TRUE|FALSE] [--dim=N] [--times=N] ",&
