program psb_z_comm_cg_test
  ...
  type(psb_z_vect_type) :: b, x
  type(psb_zprec_type)  :: prec
#ifdef PSB_HAVE_CUDA
  type(psb_z_vect_cuda) :: vmold
  type(psb_i_vect_cuda) :: imold
  type(psb_z_cuda_hlg_sparse_mat), target :: ahlg
  class(psb_z_base_sparse_mat), pointer :: agmold
#endif
  ...
  if (my_rank == psb_root_) then
    ...
    write(psb_out_unit,'(" ")')
    write(psb_out_unit,'("Usage: ./psb_z_comm_cg_test [idim] [nrep] [nwarm] [itmax] ",&
  ...
  if (use_external_matrix) then
    ...
    ! call probe_ieee('before psb_z_gen_pde3d')
    call psb_z_gen_pde3d(ctxt,idim,a,b,x,desc_a,afmt,info)
    ! call probe_ieee('after psb_z_gen_pde3d')
  end if
  ...
  do prec_idx = 1, n_precs
    do scheme_idx = 1, n_schemes
      do rep = 1, nrep
        ...
        call psb_geaxpby(zzero,b,zzero,x,desc_a,info)
        if (info /= psb_success_) goto 9999
...
contains
  ...
  subroutine psb_z_gen_pde3d(ctxt,idim,a,bv,xv,desc_a,afmt,info)
    ...
    type(psb_zspmat_type), intent(out) :: a
    type(psb_z_vect_type), intent(out) :: xv,bv
    type(psb_desc_type), intent(out)   :: desc_a
    type(psb_ctxt_type), intent(in)    :: ctxt
    integer(psb_ipk_), intent(out)     :: info
    character(len=*), intent(in)       :: afmt
    ...
    do ii=1, nlr,nb
      ...
      do k=1,ib
        ...
        val(icoeff)=2.0e0_psb_dpk_*(a1(x,y,z)+a2(x,y,z)+a3(x,y,z))/sqdeltah + cfun(x,y,z)
...
end program psb_z_comm_cg_test
