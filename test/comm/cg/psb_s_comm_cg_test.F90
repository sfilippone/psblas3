program psb_s_comm_cg_test
  ...
  if (my_rank == psb_root_) then
    ...
    write(psb_out_unit,'(" ")')
    write(psb_out_unit,'("Usage: ./psb_s_comm_cg_test [idim] [nrep] [nwarm] [itmax] ",&
  ...
  if (use_external_matrix) then
    ...
    ! call probe_ieee('before psb_s_gen_pde3d')
    call psb_s_gen_pde3d(ctxt,idim,a,b,x,desc_a,afmt,info)
    ! call probe_ieee('after psb_s_gen_pde3d')
  end if
  ...
  do prec_idx = 1, n_precs
    do scheme_idx = 1, n_schemes
      do rep = 1, nrep
        ...
        call psb_geaxpby(szero,b,szero,x,desc_a,info)
        if (info /= psb_success_) goto 9999
...
contains
  ...
  subroutine load_external_matrix(ctxt, matrix_file, matrix_fmt, a, bv, xv, desc_a, afmt, info)
    ...
    type(psb_s_vect_type), intent(out)   :: bv, xv
    type(psb_desc_type), intent(out)     :: desc_a
    character(len=*), intent(in)         :: afmt
    integer(psb_ipk_), intent(out)       :: info

    type(psb_lsspmat_type) :: aux_a
    real(psb_spk_), allocatable :: rhs_glob(:), x_glob(:)
  ...
  subroutine psb_s_gen_pde3d(ctxt,idim,a,bv,xv,desc_a,afmt,info)
    ...
    type(psb_sspmat_type), intent(out) :: a
    type(psb_s_vect_type), intent(out) :: xv,bv
    type(psb_desc_type), intent(out)   :: desc_a
    type(psb_ctxt_type), intent(in)    :: ctxt
    integer(psb_ipk_), intent(out)     :: info
    character(len=*), intent(in)       :: afmt
...
end program psb_s_comm_cg_test
