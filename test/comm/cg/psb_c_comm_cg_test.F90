program psb_c_comm_cg_test
  ...
  type(psb_c_vect_type) :: b, x
  type(psb_cprec_type)  :: prec
#ifdef PSB_HAVE_CUDA
  type(psb_c_vect_cuda) :: vmold
  type(psb_i_vect_cuda) :: imold
  type(psb_c_cuda_hlg_sparse_mat), target :: ahlg
  class(psb_c_base_sparse_mat), pointer :: agmold
#endif
  ...
  real(psb_spk_) :: eps, err, t_start, t_elapsed
  real(psb_spk_), allocatable :: prec_init_time(:,:,:), prec_bld_time(:,:,:)
  real(psb_spk_), allocatable :: comm_set_time(:,:,:), krylov_time(:,:,:)
  real(psb_spk_), allocatable :: setup_time(:,:,:), solve_time(:,:,:)
  real(psb_spk_), allocatable :: total_time(:,:,:), final_error(:,:,:)
  real(psb_spk_), allocatable :: krylov_it_time(:,:,:), total_it_time(:,:,:)
  real(psb_spk_) :: iter_denom
  real(psb_spk_) :: avg_t, std_t, med_t, p10_t, p90_t, min_t, max_t
  ...
  if (my_rank == psb_root_) then
    ...
    write(psb_out_unit,'(" ")')
    write(psb_out_unit,'("Usage: ./psb_c_comm_cg_test [idim] [nrep] [nwarm] [itmax] ",&
  ...
  if (use_external_matrix) then
    ...
    ! call probe_ieee('before psb_c_gen_pde3d')
    call psb_c_gen_pde3d(ctxt,idim,a,b,x,desc_a,afmt,info)
    ! call probe_ieee('after psb_c_gen_pde3d')
  end if
  ...
  do prec_idx = 1, n_precs
    do scheme_idx = 1, n_schemes
      do rep = 1, nrep
        ...
        call psb_geaxpby(czero,b,czero,x,desc_a,info)
        if (info /= psb_success_) goto 9999
        ...
        iter_denom = real(max(iter,1_psb_ipk_),psb_spk_)
...
contains
  ...
  subroutine compute_stats(vals,mean_v,std_v,median_v,p10_v,p90_v,min_v,max_v)
    real(psb_spk_), intent(in)  :: vals(:)
    real(psb_spk_), intent(out) :: mean_v,std_v,median_v,p10_v,p90_v,min_v,max_v
    real(psb_spk_), allocatable :: tmp(:)
    integer(psb_ipk_) :: n, idx10, idx90

    n = size(vals)
    if (n <= 0) then
      mean_v = szero; std_v = szero; median_v = szero
      p10_v  = szero; p90_v = szero; min_v = szero; max_v = szero
      return
    end if

    mean_v = sum(vals)/real(n,psb_spk_)
    if (n > 1) then
      std_v = sqrt(sum((vals-mean_v)**2)/real(n-1,psb_spk_))
    else
      std_v = szero
    end if
    ...
    if (mod(n,2) == 0) then
      median_v = (tmp(n/2)+tmp(n/2+1))/2.0_psb_spk_
    else
      median_v = tmp((n+1)/2)
    end if

    idx10 = int(ceiling(0.10_psb_spk_*real(n,psb_spk_)),kind=psb_ipk_)
    idx90 = int(ceiling(0.90_psb_spk_*real(n,psb_spk_)),kind=psb_ipk_)
  ...
  subroutine load_external_matrix(ctxt, matrix_file, matrix_fmt, a, bv, xv, desc_a, afmt, info)
    ...
    type(psb_c_vect_type), intent(out)   :: bv, xv
    type(psb_desc_type), intent(out)     :: desc_a
    character(len=*), intent(in)         :: afmt
    integer(psb_ipk_), intent(out)       :: info

    type(psb_lcspmat_type) :: aux_a
    complex(psb_spk_), allocatable :: rhs_glob(:), x_glob(:)
  ...
  subroutine psb_c_gen_pde3d(ctxt,idim,a,bv,xv,desc_a,afmt,info)
    implicit none
    integer(psb_ipk_), intent(in)     :: idim
    type(psb_cspmat_type), intent(out) :: a
    type(psb_c_vect_type), intent(out) :: xv,bv
    type(psb_desc_type), intent(out)   :: desc_a
    type(psb_ctxt_type), intent(in)    :: ctxt
    integer(psb_ipk_), intent(out)     :: info
    character(len=*), intent(in)       :: afmt
    ...
    real(psb_spk_) :: t0, t1, t2, t3, tasb, talc, ttot, tgen, tcdasb
    ...
    do ii=1, nlr,nb
      ...
      do k=1,ib
        ...
        if (ix == 1) then
          zt(k) = gfun(szero,y,z)*(-val(icoeff)) + zt(k)
        else
        ...
        if (iy == 1) then
          zt(k) = gfun(x,szero,z)*(-val(icoeff)) + zt(k)
        else
        ...
        if (iz == 1) then
          zt(k) = gfun(x,y,szero)*(-val(icoeff)) + zt(k)
        else
        ...
        val(icoeff)=2.0e0_psb_spk_*(a1(x,y,z)+a2(x,y,z)+a3(x,y,z))/sqdeltah + cfun(x,y,z)
        ...
        val(icoeff) = -a3(x,y,z)/sqdeltah+b3(x,y,z)/deltah2
        if (iz == idim) then
          zt(k) = gfun(x,y,sone)*(-val(icoeff)) + zt(k)
        else
          icol(icoeff) = (ix-1)*idim*idim+(iy-1)*idim+(iz+1)
          irow(icoeff) = glob_row
          icoeff = icoeff+1
        ...
        val(icoeff) = -a2(x,y,z)/sqdeltah+b2(x,y,z)/deltah2
        if (iy == idim) then
          zt(k) = gfun(x,sone,z)*(-val(icoeff)) + zt(k)
        else
        ...
        val(icoeff) = -a1(x,y,z)/sqdeltah+b1(x,y,z)/deltah2
        if (ix == idim) then
          zt(k) = gfun(sone,y,z)*(-val(icoeff)) + zt(k)
        else
...
9999 call psb_error_handler(ctxt,err_act)
    return
  end subroutine psb_c_gen_pde3d

end program psb_c_comm_cg_test
