!
! Test program for s-type halo exchange: baseline vs neighbor topology.
!
...
!
! Run with:  mpirun -np <P> ./psb_s_comm_test
!
program psb_s_comm_test
  use psb_base_mod
  use psb_util_mod
  ...
  type(psb_sspmat_type)               :: a_mat
  type(psb_lsspmat_type)              :: aux_a

  ! ---- vectors ----
  type(psb_s_vect_type)               :: v_baseline, v_neighbor, v_neighbor_persistent, v_rma_get, v_rma_put

  ! ---- temporary / comparison arrays ----
  real(psb_spk_), allocatable          :: vals(:)
  real(psb_spk_), allocatable          :: result_baseline(:), result_neighbor(:), result_persistent(:), &
    & result_rma_get(:), result_rma_put(:)
  real(psb_spk_), allocatable          :: expected(:)
  ...
  if (my_rank == 0) then
    write(psb_out_unit,'("================================================")')
    write(psb_out_unit,'("  Test: s-type halo  baseline vs neighbor topo")')
    ...
    if (use_external_matrix) then
      ...
      write(psb_out_unit,'("  Grid      : ",i0," x ",i0," x ",i0)') idim,idim,idim
        ...
        write(psb_out_unit,'("  Usage     : ./psb_s_comm_test [--dim N] [--iters N] [--mode ...] ",&
  ...
  if (run_neighbor) then
    ...
    call psi_swapdata(psb_comm_status_start_, szero, v_neighbor%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'neighbor start error:', info
      call psb_abort(ctxt)
    end if

    call psi_swapdata(psb_comm_status_wait_, szero, v_neighbor%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
  ...
  if (run_persistent) then
    ...
    call psi_swapdata(psb_comm_status_start_, szero, v_neighbor_persistent%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'persistent-neighbor start error:', info
      call psb_abort(ctxt)
    end if
    call psi_swapdata(psb_comm_status_wait_, szero, v_neighbor_persistent%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
  ...
  if(run_rma_get) then 
    ...
    call psi_swapdata(psb_comm_status_start_, szero, v_rma_get%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'RMA get start error:', info
      call psb_abort(ctxt)
    end if
    call psi_swapdata(psb_comm_status_wait_, szero, v_rma_get%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
    ...
    call psi_swapdata(psb_comm_status_start_, szero, v_rma_put%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'RMA put start error:', info
      call psb_abort(ctxt)
    end if
    call psi_swapdata(psb_comm_status_wait_, szero, v_rma_put%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
  ...
  do i = 1, iters
    if (run_baseline) then
      t0 = psb_wtime()
      call psi_swapdata(psb_comm_status_start_, szero, v_baseline%v, desc_a, info, data=psb_comm_halo_)
      call psi_swapdata(psb_comm_status_wait_, szero, v_baseline%v, desc_a, info, data=psb_comm_halo_)
      t1 = psb_wtime()
      dt = t1 - t0
...
contains 
  ...
  subroutine check_result(test_name, arr1, arr2, n, tolerance)
    ...
    real(psb_spk_), intent(in)  :: arr1(:), arr2(:)
