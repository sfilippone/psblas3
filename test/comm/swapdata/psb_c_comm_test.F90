!
! Test program for c-type halo exchange: baseline vs neighbor topology.
!
...
!
! Run with:  mpirun -np <P> ./psb_c_comm_test
!
program psb_c_comm_test
  use psb_base_mod
  use psb_util_mod
  ...
  type(psb_cspmat_type)               :: a_mat
  type(psb_lcspmat_type)              :: aux_a

  ! ---- vectors ----
  type(psb_c_vect_type)               :: v_baseline, v_neighbor, v_neighbor_persistent, v_rma_get, v_rma_put

  ! ---- temporary / comparison arrays ----
  complex(psb_spk_), allocatable          :: vals(:)
  complex(psb_spk_), allocatable          :: result_baseline(:), result_neighbor(:), result_persistent(:), &
    & result_rma_get(:), result_rma_put(:)
  complex(psb_spk_), allocatable          :: expected(:)
  ...
  logical                               :: comm_ok
  real(psb_spk_)                      :: err, tol
  real(psb_spk_)                      :: first_swap_baseline, first_swap_neighbor, first_swap_persistent, &
    & first_swap_rma_get, first_swap_rma_put
  real(psb_spk_)                      :: comm_setup_time_baseline, comm_setup_time_neighbor, comm_setup_time_persistent, &
    & comm_setup_time_rma_get, comm_setup_time_rma_put
  real(psb_spk_)                      :: t0, t1, dt, tsum_baseline, tsum_neighbor, tsum_neighbor_persistent, &
    & tsum_rma_get, tsum_rma_put
  integer(psb_lpk_), allocatable        :: glob_col(:)
  character(len=40)                     :: name
  real(psb_spk_)                      :: huge_d

  name    = 'psb_c_comm_test'
  tol     = 1.0e-12_psb_spk_
  huge_d  = huge(sone)
  n_pass  = 0
  n_total = 0
  iters = 5
  ...
  if (my_rank == 0) then
    write(psb_out_unit,'("================================================")')
    write(psb_out_unit,'("  Test: c-type halo  baseline vs neighbor topo")')
    ...
    if (use_external_matrix) then
      ...
      write(psb_out_unit,'("  Grid      : ",i0," x ",i0," x ",i0)') idim,idim,idim
        ...
        write(psb_out_unit,'("  Usage     : ./psb_c_comm_test [--dim N] [--iters N] [--mode ...] ",&
  ...
  do i = 1, ncol
    expected(i) = real(glob_col(i),psb_spk_)
  end do
  ...
  first_swap_baseline = szero
  first_swap_neighbor = szero
  first_swap_persistent = szero
  first_swap_rma_get = szero
  first_swap_rma_put = szero

  comm_setup_time_baseline = szero
  comm_setup_time_neighbor = szero
  comm_setup_time_persistent = szero
  comm_setup_time_rma_get = szero
  comm_setup_time_rma_put = szero
  ...
  if (run_neighbor) then
    ...
    call psi_swapdata(psb_comm_status_start_, czero, v_neighbor%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'neighbor start error:', info
      call psb_abort(ctxt)
    end if

    call psi_swapdata(psb_comm_status_wait_, czero, v_neighbor%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
  ...
  if (run_persistent) then
    ...
    call psi_swapdata(psb_comm_status_start_, czero, v_neighbor_persistent%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'persistent-neighbor start error:', info
      call psb_abort(ctxt)
    end if
    call psi_swapdata(psb_comm_status_wait_, czero, v_neighbor_persistent%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
  ...
  if(run_rma_get) then 
    ...
    call psi_swapdata(psb_comm_status_start_, czero, v_rma_get%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'RMA get start error:', info
      call psb_abort(ctxt)
    end if
    call psi_swapdata(psb_comm_status_wait_, czero, v_rma_get%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
    ...
    call psi_swapdata(psb_comm_status_start_, czero, v_rma_put%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
      write(psb_err_unit,*) my_rank, 'RMA put start error:', info
      call psb_abort(ctxt)
    end if
    call psi_swapdata(psb_comm_status_wait_, czero, v_rma_put%v, desc_a, info, data=psb_comm_halo_)
    if (info /= psb_success_) then
  ...
  tsum_baseline = szero
  tsum_neighbor = szero
  tsum_neighbor_persistent = szero
  tsum_rma_get = szero
  tsum_rma_put = szero
  ...
  do i = 1, iters
    if (run_baseline) then
      t0 = psb_wtime()
      call psi_swapdata(psb_comm_status_start_, czero, v_baseline%v, desc_a, info, data=psb_comm_halo_)
      call psi_swapdata(psb_comm_status_wait_, czero, v_baseline%v, desc_a, info, data=psb_comm_halo_)
      t1 = psb_wtime()
      dt = t1 - t0
  ...
  if (my_rank == 0) then
    if (run_baseline) then
      write(psb_out_unit,'("  Avg baseline time  : ",es12.5)') (tsum_baseline / real(iters,psb_spk_))
    ...
    if (run_neighbor) then
      write(psb_out_unit,'("  Avg neighbor time  : ",es12.5)') (tsum_neighbor / real(iters,psb_spk_))
    ...
    if (run_persistent) then
      write(psb_out_unit,'("  Avg pers-neigh time: ",es12.5)') (tsum_neighbor_persistent / real(iters,psb_spk_))
    ...
    if (run_rma_get) then
      write(psb_out_unit,'("  Avg RMA get time   : ",es12.5)') (tsum_rma_get / real(iters,psb_spk_))
    ...
    if (run_rma_put) then
      write(psb_out_unit,'("  Avg RMA put time   : ",es12.5)') (tsum_rma_put / real(iters,psb_spk_))
...
contains 
  ...
  subroutine check_result(test_name, arr1, arr2, n, tolerance)
    ...
    complex(psb_spk_), intent(in)  :: arr1(:), arr2(:)
    integer(psb_ipk_), intent(in):: n
    real(psb_spk_), intent(in) :: tolerance
    real(psb_spk_)             :: err_val
    ...
    if (my_rank == 0) then
      if ((err_val >= szero) .and. (err_val < tolerance)) then
