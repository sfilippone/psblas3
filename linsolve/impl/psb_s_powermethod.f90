subroutine psb_s_powermethod(a, prec, lambda, desc, info, x, flag, itmax, iter, tol)
    ...
    type(psb_s_vect_type)   :: z, q 
    integer(psb_ipk_)       :: i, itmax_
    real(psb_spk_)          :: tol_
    real(psb_spk_)          :: lambda_old, norm_factor
    ...
    if(present(x) .and. present(flag)) then     !TO DO: can we avoid the allocation of one vector in this case?
        if(flag) call psb_geaxpby(sone, x, szero, q, desc, info)
    end if

    lambda_old = szero
    do i = 1, itmax_
        norm_factor = sone / psb_genrm2(q, desc, info)
        call psb_geaxpby(norm_factor, q, szero, z, desc, info)      ! z_k = q_k / |q_k|_2
        call psb_spmm(sone, a, z, szero, q, desc, info)             ! q_k = A z_k 
    ...
    if(present(x)) call psb_geaxpby(sone, z, szero, x, desc, info)
