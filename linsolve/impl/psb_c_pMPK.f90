subroutine psb_c_pMPK_packd(spmat, prec, vec_in, mvec_out, s, desc, info, & 
    ...
    complex(psb_spk_)    :: gamma_
    logical           :: save_r
    character         :: base_type_
    character(len=20) :: name = "psb_c_pMPK"
    ...
    select case(base_type_)
        case ("M")
            call psb_c_pMPK_packd_monomial() 
        ...
        case ("C")
            ...
            call psb_c_pMPK_packd_chebyshev()
...
contains
    subroutine psb_c_pMPK_packd_monomial()
        implicit none
        character(len=20) :: name = "psb_c_pMPK_monomial"
        integer(psb_ipk_) :: i, idx_Z, idx_Q

        ! Copy r in the first column of Q
        idx_Q = s + 1
        call psb_geaxpby(cone, vec_in, czero, mvec_out, idx_Q, desc, info)
    ...
    end subroutine psb_c_pMPK_packd_monomial

    subroutine psb_c_pMPK_packd_chebyshev()
        implicit none
        character(len=20) :: name = "psb_c_pMPK_chebyshev"
        integer(psb_ipk_) :: i, idx_Z, idx_Q, ind_tmp
        type(psb_c_multivect_type), pointer :: mvec_tmp
        ...
        ind_tmp = 1
        call psb_geaxpby(cone, vec_in, czero, mvec_tmp, ind_tmp, desc, info)
        ...
        ind_tmp = ind_tmp + 1
        call psb_geaxpby(alpha, mvec_out, idx_Q, -beta, mvec_tmp, ind_tmp - 1, czero, mvec_tmp, ind_tmp, desc, info)
        idx_Z = idx_Z + 1
        call prec%apply(mvec_tmp, ind_tmp, mvec_out, idx_Z, desc, info, work = aux)
        idx_Q = merge(idx_Q + 1, idx_Q, save_r) !If not save_r overwrite it with the first vector
        call psb_spmm(cone, spmat, mvec_out, idx_Z, czero, mvec_out, idx_Q, desc, info, work = aux)
...
subroutine psb_c_pMPK_split(spmat, prec, vec_in, Z, Q, s, desc, info,  & 
    ...
    select case(base_type_)
        case ("M")
            call psb_c_pMPK_split_monomial() 
        ...
        case ("C")
            ...
            call psb_c_pMPK_split_chebyshev()
...
contains
    subroutine psb_c_pMPK_split_monomial()
        implicit none
        character(len=20) :: name = "psb_c_pMPK_monomial"
        integer(psb_ipk_) :: i, idx_Z, idx_Q
        ...
        ! Copy r in the first column of Q
        idx_Q = 1
        call psb_geaxpby(cone, vec_in, czero, Q, idx_Q, desc, info)
        ...
        idx_Q = merge(idx_Q + 1, idx_Q, save_r) !If not save_r overwrite it with the first vector
        call psb_spmm(cone, spmat, Z, idx_Z, czero, Q, idx_Q, desc, info, work = aux)
    ...
    end subroutine psb_c_pMPK_split_monomial

    subroutine psb_c_pMPK_split_chebyshev()
        implicit none
        character(len=20) :: name = "psb_c_pMPK_chebyshev"
        integer(psb_ipk_) :: i, idx_Z, idx_Q, ind_tmp
        type(psb_c_multivect_type), pointer :: mvec_tmp
        ...
        ! First update of temp multivector + second iteration
        ind_tmp = ind_tmp + 1
        call psb_geaxpby(alpha, Q, idx_Q, -beta, mvec_tmp, ind_tmp - 1, czero, mvec_tmp, ind_tmp, desc, info)
        idx_Z = idx_Z + 1
        call prec%apply(mvec_tmp, ind_tmp, Z, idx_Z, desc, info, work = aux)
        idx_Q = idx_Q + 1
        call psb_spmm(cone, spmat, Z, idx_Z, czero, Q, idx_Q, desc, info, work = aux)
    ...
    9998 if(.not. present(mvec_temp)) call psb_gefree(mvec_tmp, desc, info)
        return
    end subroutine psb_c_pMPK_split_chebyshev
