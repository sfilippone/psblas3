  ...
  subroutine psb_c_bjac_apply(alpha,prec,x,beta,y,desc_data,info,trans,work)
    ...
    case(psb_f_ilu_n_) 
      ...
      case('N')
        ...
        if(info == psb_success_) call psb_spsm(alpha,prec%av(psb_u_pr_),ww,beta,y,&
             & desc_data,info,trans=trans_,scale='U',choice=psb_none_, work=aux)
        
      case('T')
        call psb_spsm(cone,prec%av(psb_u_pr_),x,czero,ww,desc_data,info,&
             & trans=trans_,scale='L',diag=prec%d,choice=psb_none_, work=aux)
        if(info == psb_success_)  call psb_spsm(alpha,prec%av(psb_l_pr_),ww,beta,y,&
             & desc_data,info,trans=trans_,scale='U',choice=psb_none_,work=aux)
